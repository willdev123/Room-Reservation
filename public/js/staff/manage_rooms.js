	...
	if (rooms_data.message) {
		...
		rooms_data.result.forEach((room) => {
			...
			<div class="card-body">
				...
				<a href="/manage_edit_room/${room.id}" class="btn text-white fw-semibold bg-cusbtnprimary"
					>Detail</a
				>
			</div>
		</div>`;
...
document.querySelector("#create-room-btn").onclick = () => {
	window.location.assign("/manage_create_room");
};
